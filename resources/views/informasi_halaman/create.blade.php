...
@section('content')
<form action="{{route('informasi-halaman.store')}}" method="post" enctype="multipart/form-data">
    @csrf
    <x-adminlte-card theme="primary" theme-mode="outline" title="{{$subTitle}}">
        ...
        <div class="row">
            <x-adminlte-select name="halaman_nama" id="halaman_nama" label="Halaman" placeholder="Halaman" fgroup-class="col-md-2">
                ...
                <option value="{{$halaman}}" {{old('halaman_nama') == $halaman ? 'selected' : ''}}>{{ucfirst($halaman)}}</option>
                @endforeach
            </x-adminlte-select>
            ...
            <x-adminlte-select name="halaman_aksi" id="halaman_aksi" label="Aksi" placeholder="Aksi" fgroup-class="col-md-2">
                ...
                <option value="{{$aksi}}" {{old('halaman_aksi') == $aksi ? 'selected' : ''}}>{{ucfirst($aksi)}}</option>
                @endforeach
            </x-adminlte-select>

            <x-adminlte-input name="judul" id="judul" label="Judul" placeholder="Judul" fgroup-class="col-md-8" value="{{old('judul')}}" />
