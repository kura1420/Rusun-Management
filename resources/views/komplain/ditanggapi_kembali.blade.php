...
<div class="card card-primary card-outline">
    ...
    <form action="{{route('komplain.tanggapiKembaliStore', $row->id)}}" method="post" enctype="multipart/form-data">
        @csrf
    <div class="card-body">
        ...
        <div class="form-group">
            <div class="row">
                <div class="col-md-9">
                    <x-adminlte-input name="judul" id="judul" placeholder="Judul" value="{{$row->komplain->judul}}" disabled />
                </div>
                <div class="col-md-3">
                    <x-adminlte-select name="tingkat" id="tingkat" disabled>
                        <option value="">Pilih Tingkat</option>
                        <option value="3" {{$row->komplain->tingkat == 3 ? 'selected' : ''}}>High</option>
                        <option value="2" {{$row->komplain->tingkat == 2 ? 'selected' : ''}}>Medium</option>
                        <option value="1" {{$row->komplain->tingkat == 1 ? 'selected' : ''}}>Low</option>
                    </x-adminlte-select>
                </div>
            </div>
        </div>
