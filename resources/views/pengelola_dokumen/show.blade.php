...
<div class="row">
    <div class="col-md-4">
        ...
        <x-adminlte-card theme="danger" theme-mode="outline" title="Verifikasi Dokumen">
            <form action="{{route('pengelola-dokumen.verif', $row->id)}}" method="post">
            @csrf
            @method('PUT')
    ...
    <div class="col-md-8">
        ...
        <x-adminlte-card theme="success" theme-mode="outline" title="{{$row->dokumens->nama}}">
            <embed type="application/pdf" src="{{route('pengelola-dokumen.view_file', [$row->id, $row->file])}}" width="100%" height="800"></embed>
        </x-adminlte-card>
        @else
