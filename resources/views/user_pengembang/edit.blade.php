...
<form action="{{route('user-pengembang.update', $row->id)}}" method="post">
    @csrf
    @method('PUT')
    <x-adminlte-card theme="primary" theme-mode="outline" title="{{$subTitle}}">
        ...
        <div class="row">
            <x-adminlte-select2 name="pengembang" label="Pengembang" fgroup-class="col-md-12" :config="[
                ...
                <option value="{{$pengembang->id}}" {{$pengembang->id == $row->user_mapping->reff_id || $pengembang == $pengembang->id ? 'selected' : ''}}>{{$pengembang->nama}}</option>
                @endforeach
            </x-adminlte-select2>
