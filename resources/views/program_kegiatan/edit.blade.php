...
@section('content')
<form action="{{route('program-kegiatan.update', $row->id)}}" method="post" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <x-adminlte-card theme="primary" theme-mode="outline" title="{{$subTitle}}">
        ...
        <div class="row">
            ...
            <x-adminlte-input-date name="tanggal_mulai" label="Tanggal Mulai" placeholder="Tgl. Mulai" :config="['format' => 'YYYY-MM-DD']" value="{{$row->tanggal_mulai}}" fgroup-class="col-md-2" />
            <x-adminlte-input-date name="tanggal_berakhir" label="Tanggal Akhir" placeholder="Tgl. Akhir" :config="['format' => 'YYYY-MM-DD']" value="{{$row->tanggal_berakhir}}" fgroup-class="col-md-2" />
            ...
            <x-adminlte-select name="template" label="Template" placeholder="Template" fgroup-class="col-md-12">
                ...
                <option value="penetapan_hasil_pemilihan" {{$row->template == 'penetapan_hasil_pemilihan' ? 'selected' : ''}}>Penetapan Hasil Pemilihan</option>
            </x-adminlte-select>
