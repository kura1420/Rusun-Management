...
<form action="{{route('user-penghuni.update', $row->id)}}" method="post">
    @csrf
    @method('PUT')
    <x-adminlte-card theme="primary" theme-mode="outline" title="{{$subTitle}}">
        <x-slot name="toolsSlot">
            <a href="{{route('user-penghuni.index')}}" class="btn btn-sm btn-dark">
                <i class="fa fa-arrow-left"></i> Kembali
            </a>
        </x-slot>
        
        <div class="row">
            <x-adminlte-select2 name="rusun_penghuni" label="Penghuni" fgroup-class="col-md-12" readonly :config="[
                'placeholder' => 'Pilih Penghuni',
                'allowClear' => true,
            ]">
                <option value=""></option>
                @foreach ($penghunis as $penghuni)
                <option value="{{$penghuni->id}}" {{$penghuni->id == $row->user_mapping->reff_id || $penghuni == $penghuni->id ? 'selected' : ''}}>{{$penghuni->nama}}</option>
                @endforeach
            </x-adminlte-select2>
