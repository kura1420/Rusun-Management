...
<form action="{{route('pengembang-dokumen.store')}}" method="post" enctype="multipart/form-data">
    ...
    <x-adminlte-card theme="primary" theme-mode="outline" title="{{$subTitle}}">
        ...
        <div class="row">
            ...
            <x-adminlte-select2 name="pengembang_id" label="Pengembang" fgroup-class="col-md-4" readonly :config="[
            ...
            <x-adminlte-select2 name="rusun_id" label="Rusun" fgroup-class="col-md-4" :config="[
                ...
                <option value=""></option>
                @foreach ($rusunPengelolas as $key => $rusunPengelola)
                <option value="{{$rusunPengelola->rusun_id}}" {{old('rusun_id') == $rusunPengelola->rusun_id ? 'selected' : ''}}>{{$rusunPengelola->rusun->nama}}</option>
                @endforeach
            </x-adminlte-select2>
