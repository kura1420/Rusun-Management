    ...
    <x-adminlte-card theme="primary" theme-mode="outline" title="{{$subTitle}}">
        ...
        <div class="row">
            ...
            <x-adminlte-select2 name="tahun" label="Tahun Awal Jabatan" fgroup-class="col-md-3" :config="[
                    ...
                    <option value="{{$tahun}}" {{$tahun == old('tahun') || date('Y') == $tahun ? 'selected' : ''}}>{{$tahun}}</option>
                    @endforeach
            </x-adminlte-select2>

            <x-adminlte-input type="number" name="periode" id="periode" label="Periode Jabatan (Tahun)" placeholder="Periode Jabatan (Tahun)" fgroup-class="col-md-3" value="{{old('periode')}}" />
